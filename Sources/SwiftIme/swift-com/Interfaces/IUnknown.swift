...
open class IUnknown {
    ...
    public required init(pUnk pointer: UnsafeMutableRawPointer?) {
        self.pUnk = IUnknownRef(
            consuming: pointer?.bindMemory(to: WinSDK.IUnknown.self, capacity: 1))
    }
...
extension IUnknown {
    public static func CreateInstance<Interface: IUnknown>(
        `class` clsid: CLSID,
        ...
        throws -> Interface
    {
        ...
        try CHECKED(
            CoCreateInstance(
                &clsid, RawPointer(pUnkOuter), DWORD(dwClsContext.rawValue), &iid, &pointer))
        return Interface(pUnk: pointer)
    }
}
extension IUnknown {
    func perform<Type, ResultType>(
        as type: Type.Type,
